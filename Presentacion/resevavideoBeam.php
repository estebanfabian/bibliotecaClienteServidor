...
<html>
    ...
    <body>.bg{background-color:red}<!-- Navigation -->
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <form id="PrestamoVideoBeamComputador" name ="PrestamoVideoBeamComputador" style="margin-bottom: 10px;">
                        <fieldset class="border p-1">
                            <legend class="w-auto">Prestamo video beam</legend>
                            <div class="form-row">
                                <div class="form-group col-md-4">
                                    <label for="inputEmail4">numero de video beam (*)</label>
                                    <select type="tetx" class="form-control" id="NumeroVideoBeam" name ="NumeroVideoBeam" placeholder="numero del video beam "> </select>
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="inputPassword4">Fecha de prestamo (*)</label> 
                                    <input type="datetime-local" class="form-control" id="fechaPrestamo1" name="fechaPrestamo1"required="" >
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="inputPassword4">Fecha de entrega (*)</label> 
                                    <input type="datetime-local" class="form-control" id="fechaPrestamo2" name="fechaPrestamo2" required=""> 
                                </div>

                                <div class="form-group col-md-4">
                                    <label for="inputEmail4">numero computador (*)</label>
                                    <select type="tetx" class="form-control" id="Numerocomputador" name ="codigoUsuario" placeholder="numero del computador"></select>
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="inputEmail4">Observaciones</label>
                                    <input type="tetx" class="form-control" id="ObservacionesPVideoBeam" name ="codigoUsuario" placeholder="Observaciones">
                                </div>
                                <?php if ($_SESSION['usuario']['perfil'] == 'administrador' || $_SESSION['usuario']['perfil'] == 'empleado') { ?>
                                    <div class="form-group col-md-4">
                                        <label for="inputEmail4">Codigo del prestador(*)</label>
                                        <input type="tetx" class="form-control" id="CoddigoPrestamo" name ="codigoUsuario" placeholder="numero del video beam ">
                                    </div>
                                <?php } ?>
                                <div class="form-group col-md-12">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="exampleCheck1">
                                        <label class="form-check-label" for="exampleCheck1">con computador</label>
                                    </div>
                                </div> </div>
                        </fieldset> 
                        <?php if ($_SESSION['usuario']['perfil'] == 'administrador' || $_SESSION['usuario']['perfil'] == 'empleado') { ?>
                            <button class="btn btn-primary" id="btnPrestarVideo">Prestar</button>
                        <?php } ?>
                        <button class="btn btn-primary" id="btnReservaVideo">Reservar</button>
                        <button class="btn btn-primary" id="BtnValidarVideoBeam">Validar</button>
                    </form>
                </div>
                <div class="col-md-3">
...
<script>
    ...
    function cargarVideoBeam() {
        var url = "CargarVideoBeam";
        var parametro = "";
        var metodo = function (respuesta) {
            var data = $.parseJSON(respuesta);
            if (data.length != 0) {
                for (var i = 0; i < data.length; i++) {
                    $("#NumeroVideoBeam").append('<option value=' + data[i]['idVideoBeam'] + '>' + data[i]['idVideoBeam'] + '</option>');
                }
            }
        };
        fajax(url, parametro, metodo);
    }
    function cargarcomputador() {
        var url = "CargarComputador";
        var parametro = "";
        var metodo = function (respuesta) {
            var data = $.parseJSON(respuesta);
            if (data.length != 0) {
                for (var i = 0; i < data.length; i++) {
                    $("#Numerocomputador").append('<option value=' + data[i]['idcomputador'] + '>' + data[i]['idcomputador'] + '</option>');
                }
            }
        };
        fajax(url, parametro, metodo);
